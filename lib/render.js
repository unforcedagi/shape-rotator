...
// Rotate a whole [x,y,z][] point list, reusing a scratch buffer. `mirror`
// negates z on the way in, i.e. rotates the DEPTH-MIRRORED object; paired with
// spinM(..., true) it produces the other member of the ambiguous pair.
export function rotateAll(pts, m, out, mirror) {
  const dst = out && out.length === pts.length ? out : pts.map(() => [0, 0, 0]);
  if (!mirror) { for (let i = 0; i < pts.length; i++) applyM(m, pts[i], dst[i]); return dst; }
  for (let i = 0; i < pts.length; i++) {
    const p = pts[i], o = dst[i], z = -p[2];
    o[0] = m[0] * p[0] + m[1] * p[1] + m[2] * z;
    o[1] = m[3] * p[0] + m[4] * p[1] + m[5] * z;
    o[2] = m[6] * p[0] + m[7] * p[1] + m[8] * z;
  }
  return dst;
}

// The spin matrix for a stimulus that turns about an axis given by a tilt.
// `mirror` asks for the twin: conjugating by the depth flip D = diag(1,1,-1)
// turns a rotation about k by a into a rotation about Dk by -a, so the twin is
// that rotation applied to the mirrored object. Every sign flips an even
// number of times on the way to x and y, so the projection is identical to the
// bit — which is the claim the twins strip on the app page is making.
export function spinM(tiltDeg, plane, angle, mirror) {
  const k = axisFrom(tiltDeg, plane);
  // wrap: a turn of exactly 2pi is the identity, but sin(2pi) is -2.4e-16, and
  // that is enough to reorder a depth sort and make the last frame of a loop
  // differ from the first by a pixel or two.
  const a = angle % TAU;
  if (!mirror) return rotMatrix(k, a);
  k[2] = -k[2];
  return rotMatrix(k, -a);
}
